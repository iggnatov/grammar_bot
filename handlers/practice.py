...
class Practice:
    ...
    def get_final_message(result):
        if 0 < result <= 3:
            return f"Ты завершил тренировку. Твой результат {result} слов из 16. У тебя всё ещё впереди. Дорогу осилит идущий. Попробуем ещё разок?"
        elif 4 <= result <= 6:
            return f"Дерзай! Ты движешься в правильном направлении. {result} слов из 16. Главное, не забывать о повторении. Ещё попытка?"
        elif 7 <= result <= 10:
            return f"Твой результат {result} слов из 16! Неплохо. Я в тебя верю. Может, ещё одну тему;)?"
        elif 11 <= result <= 13:
            return f"Молодец! {result} слов из 16! Осталось несколько шагов до идеала. У меня есть для тебя персональная тренировка. Готов?"
        elif 14 <= result <= 16:
            return f"{result} слов из 16! Так держать! В наше время такого знатока языка редко встретишь. А новую тему потянешь?"